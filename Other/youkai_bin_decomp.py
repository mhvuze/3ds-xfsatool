...
lines = struct.unpack("<I", str(data[0:4]))[0]
string_table_offset = struct.unpack("<I", str(data[4:8]))[0]
string_table_size = struct.unpack("<I", str(data[8:12]))[0]
...
if (key_table_offset % 16) != 0:
    key_table_offset += 16 - (key_table_offset % 16)

key_table_size = struct.unpack("<I", str(data[key_table_offset:key_table_offset + 4]))[0]
key_count = struct.unpack("<I", str(data[key_table_offset + 4:key_table_offset + 8]))[0]
key_offset = struct.unpack("<I", str(data[key_table_offset + 8:key_table_offset + 12]))[0]
key_string_length = struct.unpack("<I", str(data[key_table_offset + 12:key_table_offset + 16]))[0]
...
for i in range(0, key_count):
    crc32 = struct.unpack("<I", str(data[key_table_offset + 0x10 + (i * 8):key_table_offset + 0x10 + (i * 8) + 4]))[0]
    offset = \
        struct.unpack("<I", str(data[key_table_offset + 0x10 + (i * 8) + 4:key_table_offset + 0x10 + (i * 8) + 8]))[0]
...
while i < len(code_data) and cnt < lines:
    ...
    crc32 = struct.unpack("<I", str(code_data[i:i + 4]))[0]
    param_info = struct.unpack("<H", str(code_data[i + 4:i + 6]))[0]
    param_type = param_info >> 8
    param_count = param_info & 0xff
    terminator = struct.unpack("<H", str(code_data[i + 6:i + 8]))[0]
    ...
    for x in range(0, param_count):
        param = struct.unpack("<I", str(code_data[i:i + 4]))[0]
        type = (param_type >> (2 * x)) & 0x03

        if type == 1:  # Integer
            output += "%d" % param
        elif type == 2:  # Float
            param = struct.unpack("<f", str(code_data[i:i + 4]))[0]
